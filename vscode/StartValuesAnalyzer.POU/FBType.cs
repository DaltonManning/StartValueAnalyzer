...
public class FBType : POUType
{
	...
	protected Dictionary<string, ObjInst> multiParameters;
	...
	public override IDictionary<string, ObjInst> MultiParameters => multiParameters;
		...
		: base(name)
	{
		...
		multiParameters = new Dictionary<string, ObjInst>(StringComparer.OrdinalIgnoreCase);
	}
	...
	{
		Parameter parameter = new Parameter(name);
		if (!AppendObjInstAndSetFather(parameter, multiParameters, this))
		{
			parameter = null;
		}
		return parameter;
	...
	public override void BuildTypeStruct()
	{
		...
		BuildListStruct(multiParameters);
	...
	public override ObjInst FindInstance(string name)
	{
		...
		objInst = FindSubObject(multiParameters, name);
		if (objInst != null)
		{
			return objInst;
		}
